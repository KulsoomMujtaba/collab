...
import { CollaborationOverviewList } from "@/components/workspace/collaboration-overview-list";
...
export default async function CollaborationsPage() {
  ...
  return <main className="min-h-screen bg-background">
    ...
    <div className="mx-auto grid max-w-[1440px] gap-9 px-5 py-10 sm:px-8 lg:grid-cols-[210px_1fr] lg:py-12">
      <WorkspaceNav role={role} active="collaborations" />
      <section className="min-w-0"><p className="text-sm font-bold uppercase tracking-[.16em] text-primary">Active work</p><h1 className="display mt-2 text-4xl font-extrabold sm:text-5xl">Collaborations</h1><p className="mt-3 max-w-2xl leading-7 text-muted">See what needs your attention, continue conversations, and follow each collaboration through delivery.</p><div className="mt-8"><CollaborationOverviewList items={items} role={role} loadError={error?.message} /></div></section>
    </div>
  </main>;
}
