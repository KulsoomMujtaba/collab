...
export default async function CompanyMarketplacePage() {
  ...
  return <main className="min-h-screen bg-background"><header className="border-b border-border bg-surface"><div className="mx-auto flex h-18 max-w-[1440px] items-center justify-between px-5 sm:px-8"><Link href="/" className="display text-2xl font-extrabold text-primary">Collab<span className="text-accent">.</span></Link><div className="flex items-center gap-3"><p className="hidden text-sm text-muted sm:block">{account.full_name}</p><SignOutButton /></div></div></header><div className="mx-auto grid max-w-[1440px] gap-9 px-5 py-9 sm:px-8 lg:grid-cols-[210px_1fr] lg:py-12"><div><WorkspaceNav role="company" active="home" /><div className="mt-8 hidden rounded-2xl bg-primary p-5 text-primary-foreground lg:block"><p className="text-xs font-bold uppercase tracking-widest text-accent">Good partnerships</p><p className="mt-3 text-sm font-semibold leading-6">Start with audience fit and a clear point of view—not follower count alone.</p></div></div><section className="min-w-0"><p className="text-sm font-bold uppercase tracking-[.16em] text-primary">Creator marketplace</p><h1 className="display mt-2 text-4xl font-extrabold sm:text-5xl">Find your next collaborator.</h1><p className="mt-3 max-w-2xl leading-7 text-muted">Explore credible B2B voices and compare the numbers that matter before you reach out.</p><div className="mt-8"><CreatorMarketplace creators={creators} niches={niches} loadError={creatorResult.error?.message ?? nicheResult.error?.message} /></div></section></div></main>;
}
