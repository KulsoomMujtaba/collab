...
export function FormField({ label, error, hint, className, ...props }: BaseProps & InputHTMLAttributes<HTMLInputElement>) {
  const id = props.id ?? props.name;
  return (
    <label className="block" htmlFor={id}>
      <span className="mb-2 block text-sm font-semibold">{label}</span>
      <input id={id} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} className={cn("h-12 w-full rounded-xl border bg-surface px-4 text-[15px] outline-none transition placeholder:text-muted/60 focus:border-primary focus:ring-3 focus:ring-primary/10", error ? "border-destructive" : "border-border", className)} {...props} />
