...
function Success({ creatorName, close }: { creatorName: string; close: () => void }) { return <div className="flex min-h-[65vh] flex-col items-center justify-center text-center"><div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#e9f7f0] text-success"><CheckCircle2 size={31} /></div><p className="mt-6 text-sm font-bold uppercase tracking-[.15em] text-primary">Request sent</p><h3 className="display mt-2 text-3xl font-extrabold">It&apos;s with {creatorName}.</h3><p className="mt-3 max-w-sm leading-7 text-muted">Your brief and the creator&apos;s current rate are locked in. You&apos;ll be able to track the response from your company workspace.</p><button onClick={close} className="mt-7 inline-flex h-11 items-center justify-center rounded-full bg-primary px-6 text-sm font-semibold text-primary-foreground hover:bg-primary-hover">Done</button></div>; }
